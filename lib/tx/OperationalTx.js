...
OperationalTx.prototype.selectCoins = function (colorValue, feeEstimator, cb) {
  ...
  Q.fcall(function () {
    ...
    var computeRequiredSum = function () {
      if (feeEstimator !== null)
	return colorValue.plus(feeEstimator.estimateRequiredFee({txIns: selectedCoins.length}))
      else
	return colorValue
    }
    ...
    coins.forEach(function (coin) {
      promise = promise.then(function () {
        if (selectedCoinsColorValue.getValue() >= computeRequiredSum().getValue()) {
          return
        }
	...
	console.log(selectedCoins)
      ...
      var required = computeRequiredSum().getValue()
